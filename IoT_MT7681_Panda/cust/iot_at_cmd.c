...
extern UINT8 Pop_lock2;
extern UINT8 Pop_lock1;


#if (UART_INTERRUPT != 1)
UCHAR command[AT_CMD_MAX_LEN];
INT16 cmd_len = 0;
UCHAR sh_command[SH_CMD_STD_LEN];
#endif
...
INT16 IoT_exec_AT_cmd_uart(PUCHAR command_buffer, INT16 at_cmd_len)
{
...
INT16 IoT_exec_AT_cmd_ATE_Cal2(PUCHAR command_buffer, INT16 at_cmd_len)
{
...
INT16 IoT_exec_AT_cmd_ATE_Cal(PUCHAR cmd_buf, INT16 Len)
{
	char  Cmd[16];
	INT16 CmdLen   = 0;
	char *Data	   = NULL;
...
INT16 IoT_exec_AT_cmd_Flash_Set(PUCHAR command_buffer, INT16 at_cmd_len)
{
	char *argv[MAX_OPTION_COUNT];
	char *opString = "r:s:v:?";
	char *endptr = NULL;
...
INT16 IoT_exec_AT_cmd_Efuse_Set(PUCHAR command_buffer, INT16 at_cmd_len)
{
	char *argv[MAX_OPTION_COUNT];
	char *opString = "r:s:v:?";
	char *endptr = NULL;
...
INT16 IoT_exec_AT_cmd_PS_Set(PUCHAR command_buffer, INT16 at_cmd_len)
{
	char *argv[MAX_OPTION_COUNT];
	char *opString = "r::l:t:?";
	char *endptr = NULL;
...
INT16 IoT_exec_AT_cmd_netmode(PUCHAR command_buffer, INT16 at_cmd_len)
{
...
========================================================================*/
INT16 IoT_exec_AT_cmd_ch_switch(PUCHAR pCmdBuf, INT16 at_cmd_len)
{
	char *argv[MAX_OPTION_COUNT];
	char *opString = "b:c:?";
	char *endptr = NULL;
...
========================================================================*/
INT16 IoT_exec_AT_cmd_Conf_SoftAP(PUCHAR pCmdBuf, INT16 at_cmd_len)
{
	INT16 argc = 0;
	char *argv[MAX_OPTION_COUNT];
	...
	if (UpdateCfg)
		APCfgUpdate(SSID, Auth_Mode, Password, Channel);
...
/*========================================================================
	...
	Note:
========================================================================*/
INT16 IoT_exec_AT_Set_Smnt(PUCHAR pCmdBuf)
{
...
/*========================================================================
	...
	Note:
========================================================================*/
INT16 IoT_exec_AT_cmd_ver(PUCHAR pCmdBuf)
{
	size_t  len=0 , len2 =0;
...
VOID IoT_exec_AT_cmd_jtag_mode_switch(PUCHAR pCmdBuf, INT16 at_cmd_len)
{
	INT16 argc = 0;
	char *argv[MAX_OPTION_COUNT];
	char *opString = "m:?";
...
INT16 IoT_exec_AT_cmd_Fac_Mac(PUCHAR pCmdBuf, INT16 at_cmd_len)
{
	PCHAR argv[MAX_OPTION_COUNT];
	CHAR *opString = "m:?";
	INT32 len,i;
	INT16 argc = 0;
	CHAR opt = 0;
	...
	{	
		...
		{
			case 'm':
				parseMacString(optarg,&gCurrentAddress[0]);
				macStrConvert();
				spi_flash_write(FLASH_MAC_ADDR_OFFSET, &gCurrentAddress[0], MAC_ADDR_LEN);
				break;
			case '?':
			default:
				break;
	...
	IoT_uart_output("FacSuc\n",7);
	return 0;
}


INT16 IoT_exec_AT_cmd_Fac_OEM(PUCHAR pCmdBuf, INT16 at_cmd_len)
{
	char *argv[MAX_OPTION_COUNT];
	char *opString = "c:C:p:P:?";
	UINT8 len,needWrite=0;
	INT16 argc = 0;
	char opt = 0;
	...
	{	
		len=strlen(optarg);
		len=min(len,FLASH_USR_CFG_VENDOR_NAME_LEN);	
		switch (opt)
		{
			case 'c':
				memset(IoTpAd.UsrCfg.VendorID,0,FLASH_USR_CFG_VENDOR_ID_LEN);
				memcpy(IoTpAd.UsrCfg.VendorID,optarg,len);	
				needWrite=1;
				break;
			case 'C':
				memset(IoTpAd.UsrCfg.VendorName,0,FLASH_USR_CFG_VENDOR_NAME_LEN);
				memcpy(IoTpAd.UsrCfg.VendorName,optarg,len);	
				needWrite=1;				
				break;
			case 'p':
				memset(IoTpAd.UsrCfg.ProductType,0,FLASH_USR_CFG_PRODUCT_TYPE_LEN);
				memcpy(IoTpAd.UsrCfg.ProductType,optarg,len);
				needWrite=1;				
				break;
			case 'P':
				memset(IoTpAd.UsrCfg.ProductName,0,FLASH_USR_CFG_PRODUCT_NAME_LEN);
				memcpy(IoTpAd.UsrCfg.ProductName,optarg,len);
				needWrite=1;				
				break;				
			case '?':
			default:
	...
	if(needWrite){
		restore_usr_cfg();
		IoT_uart_output("OEMSuc\n",7);
	}
	return 0;
}
#if	(ATCMD_DEVICE_INFO_DUMP)

INT16 IoT_exec_AT_cmd_FOTA_I() XIP_ATTRIBUTE(".xipsec0");
INT16 IoT_exec_AT_cmd_InfoDump() XIP_ATTRIBUTE(".xipsec0");


INT16 IoT_exec_AT_cmd_FOTA(PUCHAR pCmdBuf, INT16 at_cmd_len)
{
	char *argv[MAX_OPTION_COUNT];
	char *opString = "u:i:?";
	INT16 argc = 0;
	...
	{	
		...
		{
			case 'i':
				if(IoTStaFOTACfg.step==FOTA_STEP_CHECK_SUCCESS&&IoTStaFOTACfg.vernumber>VER_NUMBER){
					Printf_High("%s is ready\n",IoTStaFOTACfg.vercode);
				}else{
					IoT_uart_output("not available\n",14);
				}
				break;
			case 'u':
				if(IoTStaFOTACfg.step==FOTA_STEP_CHECK_SUCCESS&&IoTStaFOTACfg.vernumber>VER_NUMBER){
					IoTStaFOTACfg.fotado=1;
					store_sta_cfg();
					Printf_High("Update to %s\n",IoTStaFOTACfg.vercode);
					Sys_reboot();
				}else{
					IoT_uart_output("not available\n",14);
				}
			...
			case '?':
			default:
				//FUNC_LOG("Please add a space at the end.\n");
				break;
		}
		opt = getopt(argc, argv, opString);
	}
...
extern UINT8 _gFOTACheck;
extern UINT8 gDNSResolvStatus;
extern u16_t serveripaddr[2];
extern IOT_ADAPTER   IoTpAd;
extern IoTLightStatus gLightStatus;

INT16 IoT_exec_AT_cmd_InfoDump()
{
	Printf_High("\n******Device Info******\n");
	Printf_High("VerCode:%s\n",__DEVELOPMENT_VERSION__);
	Printf_High("MCU:%s\n",gMCUVerCode);
	Printf_High("MAC:%s\n",gCurrentMAC);
	Printf_High("DeviceID:%s\n",IoTpAd.UsrCfg.DeviceID);
	Printf_High("SerialCode:%s\n",gSerialCode);
	Printf_High("heap start:%d,end:%d\n",&_BSS_END,&_RAM_SIZE);
	Printf_High("DNS status:%d,ip: %d.%d.%d.%d\n",gDNSResolvStatus,
		uip_ipaddr1(serveripaddr), uip_ipaddr2(serveripaddr),
		uip_ipaddr3(serveripaddr), uip_ipaddr4(serveripaddr));	
	Printf_High("SSID: %s\n", pIoTStaCfg->Ssid);
	Printf_High("pID:%s, pName:%s, cID:%s, cName:%s\n", 
		IoTpAd.UsrCfg.ProductType, IoTpAd.UsrCfg.ProductName, IoTpAd.UsrCfg.VendorID, IoTpAd.UsrCfg.VendorName);
	Printf_High("******Device Info******\n\n");
	return 0;
}
INT16 IoT_exec_AT_cmd_MemDump()
{
	xMem1stLevInfoDump();
	xMem2ndLevInfoDump();
	return 0;
...
#if __PID_GATEWAY__
INT16 IoT_exec_AT_cmd_ClearGroup()
{
	int i = 0;
	gWifiOutletCfg.Group = 0xFF;
	for(i = 0; i<8; i++)
	{
		gWifiOutletCfg.Member[i] = 0xFFFFFFFF;
		gWifiOutletCfg.LightType[i] = 0xFFFFFFFF;
	}
	outlet_flash_write(PT_WOL_MEMBERADD);
	return 0;
}
extern IoTRuleCfg gIoTRuleCfg;

INT16 IoT_exec_AT_cmd_ClearRule()
{
	int i=0;
	gIoTRuleCfg.gRuleID = 0xFFFF;
	for(i =0 ;i < 16; i++)
	{
		gIoTRuleCfg.RuleValue[i] = 0xFFFFFFFF;
		gIoTRuleCfg.RuleTODO[i] = 0xFFFFFFFF;
		gIoTRuleCfg.RulePeriod[i] = 0xFF;
	}
	SaveRule();
}
INT16 IoT_exec_AT_cmd_InfoRule()
{
	//rule_log();
}
#endif
...
#endif
INT16 IoT_parse_ATcommand(PUCHAR cmd_buf, INT16 at_cmd_len)
{
	INT16 ret_code = 0;   
#if (ATCMD_CH_SWITCH_SUPPORT == 1)
    INT8 cTypeLen = 0;
	...
	else if(!memcmp(cmd_buf,AT_CMD_WR_SERIAL,sizeof(AT_CMD_WR_SERIAL)-1))
	{
		ret_code = IoT_exec_AT_cmd_Serial(cmd_buf,at_cmd_len);
	}
#if (ATCMD_SET_SMNT_SUPPORT == 1)
...
/*Only for Debug*/
	...
	else if(!memcmp(cmd_buf,AT_CMD_FAC_IDMAC,sizeof(AT_CMD_FAC_IDMAC)-1))
	{
		ret_code = IoT_exec_AT_cmd_Fac_Mac(cmd_buf, at_cmd_len);
	}
	else if(!memcmp(cmd_buf,AT_CMD_FAC_OEM,sizeof(AT_CMD_FAC_OEM)-1))
	{
		ret_code = IoT_exec_AT_cmd_Fac_OEM(cmd_buf, at_cmd_len);
	}
#if	(ATCMD_DEVICE_INFO_DUMP)	
	else if(!memcmp(cmd_buf,AT_CMD_FOTA,sizeof(AT_CMD_FOTA)-1))
	{
		ret_code = IoT_exec_AT_cmd_FOTA(cmd_buf, at_cmd_len);
	}
	else if(!memcmp(cmd_buf,AT_CMD_InfoDump,sizeof(AT_CMD_InfoDump)-1))
	{
		ret_code = IoT_exec_AT_cmd_InfoDump();
	}
	else if(!memcmp(cmd_buf, AT_CMD_MemDump, sizeof(AT_CMD_MemDump)-1 ))
	{
		ret_code = IoT_exec_AT_cmd_MemDump();
	}
	#if __PID_GATEWAY__
	else if(!memcmp(cmd_buf, AT_CMD_CLEAROUTET, sizeof(AT_CMD_CLEAROUTET)-1))
	{
		ret_code = IoT_exec_AT_cmd_ClearGroup();
	}else if(!memcmp(cmd_buf, AT_CMD_CLEARRULE, sizeof(AT_CMD_CLEARRULE)-1)){
		ret_code = IoT_exec_AT_cmd_ClearRule();
	}else if(!memcmp(cmd_buf, AT_CMD_INFORULE, sizeof(AT_CMD_INFORULE)-1)){
		ret_code = IoT_exec_AT_cmd_InfoRule();
	}
	#endif
#endif	
...
INT16 IoT_parse_IWCmd(PUCHAR cmd_buf, INT16 at_cmd_len)
{
	INT16 ret_code = 0;   
...
INT16 IoT_parse_SHCmd(PCHAR cmd_buf)
{
	UINT8 i,ch;
	UINT8 cmd_type;
	UINT8 cs1,cs2=0;
	...
	cs1=_hexToByte(&cmd_buf[SH_CMD_STD_LEN-2],2);

	for(i=0;i<SH_CMD_STD_LEN-2;i++){
		cs2+=cmd_buf[i];
	}
	...
	cmd_type=_hexToByte(&cmd_buf[0],2);

	switch(cmd_type){
		case SHCMD_LGT_STATUS_RESP:
			#if __PID_LIGHT__
			gLightStatus.power=_hexToByte(&cmd_buf[2],2);
			gLightStatus.brightness=_hexToByte(&cmd_buf[4],2)*100;
			gLightStatus.ctemp=_hexToByte(&cmd_buf[6],2);
			gLightStatus.color=(_hexToByte(&cmd_buf[8],2)<<16)|(_hexToByte(&cmd_buf[10],2)<<8)|_hexToByte(&cmd_buf[12],2);
			#endif
		break;
		case SHCMD_MCU2WIFI_ACK:
			//FUNC_LOG("SHCMD_MCU2WIFI_ACK------------------%d\n", gWifiOutletAckOption);
			#if __PID_LIGHT__
			gLightSetOption=0;
			#endif
			#if __PID_GATEWAY__
			if(gWifiOutletAckOption != 1){
				gWifiOutletAckOption = 1;
				Pop_lock2 = 1;
				Pop_lock1 = 0;
			}
			#endif
		break;
		case SHCMD_VERCODE:
			memcpy(gMCUVerCode,&cmd_buf[SH_CMD_TYPE_LEN],
				(cmd_buf[SH_CMD_TYPE_LEN+4]=='.')?(MCU_VERCODE_LEN-1):MCU_VERCODE_LEN);
		break;
		case SHCMD_DEFAULT:
			reset_cfg();
			IoT_Cmd_LinkDown(0);
		...
		break;	
		#if __PID_GATEWAY__
		case SHCMD_WOL_STATUS_RESP:
			gWifiOutletStatus.power = _hexToByte(&cmd_buf[2], 2);
		break;
		#endif
	}
	return 0;
