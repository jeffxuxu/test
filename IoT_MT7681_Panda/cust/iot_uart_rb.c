...
{  
    ...
    static UCHAR  sh_command[SH_CMD_STD_LEN]= {0};
/*
        ...
        switch (rx_desc->cur_type)
        {
            ...
            {  
    		   ...
    		   //support more ATcmd prefix analysis
				...
				if(PKT_UNKNOWN==rx_desc->cur_type){
					if(SHCmdWraper==ch){
						SHMatchNum++;
						if(SHMatchNum==SH_CMD_STD_LEN+2){
							if(roomleft>=SH_CMD_STD_LEN){
			    			    rx_desc->cur_num = rx_desc->pkt_num;                  
			    				infor = &(rx_desc->infor[rx_desc->cur_num]);
			    				infor->pkt_len = SH_CMD_STD_LEN;								
								for(i=0;i<SH_CMD_STD_LEN;i++){
									Buf_Push(rx_ring,sh_command[i]);
								}
								infor->pkt_type = PKT_SHCMD;								
			                    rx_desc->pkt_num++;
								roomleft-=SH_CMD_STD_LEN;								
							}
						...
						if(ch == '\n' || ch == '\r'){
							...
							SHMatchNum++;
							if(SHMatchNum>SH_CMD_STD_LEN+1){
								SHMatchNum=0;
							}
						}
					}
