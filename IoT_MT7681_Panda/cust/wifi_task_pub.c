...
#define WSINIT_GO_SMNT	0
#define WSINIT_GO_SCAN	3
...
STA_FOTA_CONFIG IoTStaFOTACfg={0};


/*to indicated wheter store the SMNT setting after Wifi Connected AP and Got IP*/
BOOLEAN gSmnted = FALSE;
UCHAR _gWSInitGo= WSINIT_GO_SMNT;
WIFI_STATE _gWSPreStatus = WIFI_STATE_INIT;
...
char gCurrentMAC[MAC_ADDR_LEN*3]={0};
CHAR gSerialCode[SERAIL_CODE_LEN+1]={0};
CHAR gMCUVerCode[MCU_VERCODE_LEN+1]={0};
...
wifiTASK_LowPrioTask (VOID)
{
	...
	if (IoTCustOp.IoTCustSubTask1 != NULL)
		{	
			...
			if(tryFOTA){ 
				tryFOTA=FALSE;		
				if(IoTStaFOTACfg.fotado==1){
					IoTStaFOTACfg.fotado=0;
					if(flash_crc16_ccitt(FLASH_OFFSET_AP_FW_START,IoTStaFOTACfg.imagesize)==IoTStaFOTACfg.crc16value)
						IoTStaFOTACfg.fotaresult=IoT_OTA_Update_FW();
					else{
						IoTStaFOTACfg.fotaresult=-4;
					}
					if(IoTStaFOTACfg.fotaresult == 0)
					{
						IoTStaFOTACfg.step = FOTA_STEP_UPDATE_COMPLETE;
					}
					else
					{
						IoTStaFOTACfg.step = FOTA_STEP_NONE;
					}
					store_sta_cfg();
				}
			}
	...
	{ 
		...
		if	(gCaliEnabled == TRUE)
			...
			Printf_High("[RTask]%u \n", PreTime);
#else	
		Printf_High("[WTask]%u, [WS]%d-->%d \n", PreTime,pIoTMlme->PreWifiState,pIoTMlme->CurrentWifiState);
#endif
...
VOID wifi_state_machine(VOID)
{
	...
	if(WIFI_STATE_CONNED==pIoTMlme->PreWifiState&&
		WIFI_STATE_INIT==pIoTMlme->CurrentWifiState){
	...
	switch (pIoTMlme->CurrentWifiState)
	{
		...
		case WIFI_STATE_SCAN:
			/*Printf_High("---- AuthMode=%d,SsidLen=%d,PassphaseLen=%d\n", pIoTStaCfg->AuthMode , pIoTStaCfg->SsidLen,pIoTStaCfg->PassphaseLen);
			dump(pIoTStaCfg->Ssid,pIoTStaCfg->SsidLen);
		...
		case WIFI_STATE_CONNED:
			if((WIFI_STATE_4WAY==_gWSPreStatus||WIFI_STATE_ASSOC==_gWSPreStatus)){
				if(WSINIT_GO_SMNT==_gWSInitGo){
					//udp_broadcast_start();
					gSMNTflag = 1;
				}else{
					gSMNTflag = 0;
...
#endif
	_gWSPreStatus=pIoTMlme->CurrentWifiState;
#ifdef CONFIG_SOFTAP
#if CFG_SUPPORT_TCPIP
	tcpip_periodic_timer();
#endif
...
VOID ws_init(OUT BOOLEAN *pb_enable)
{
	*pb_enable = TRUE;
	...
	if(pIoTMlme->ValidFlashStaCfg == TRUE){
		_gWSInitGo=(_gWSInitGo+1)%WSINIT_GO_SCAN;
	}else{
		_gWSInitGo= WSINIT_GO_SMNT;
	}
	
	if (WSINIT_GO_SMNT!=_gWSInitGo&&(pIoTMlme->ValidFlashStaCfg == TRUE) &&
...
BOOLEAN load_sta_cfg(VOID)
{
	...
	if (IoTpAd.flash_rw_buf[FLASH_STA_CFG_SMNT_INFO_STORED] == SMNT_INFO_STORED)
	{
		...
		memcpy(&IoTStaFOTACfg,	&IoTpAd.flash_rw_buf[FLASH_STA_CFG_FOTA_STEP],	sizeof(STA_FOTA_CONFIG));
...
{	
	...
	memcpy(&IoTpAd.flash_rw_buf[FLASH_STA_CFG_FOTA_STEP],	&IoTStaFOTACfg,		sizeof(STA_FOTA_CONFIG));
