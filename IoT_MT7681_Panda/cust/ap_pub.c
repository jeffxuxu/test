...
/* This function will be called in APStartUp()*/
VOID APAuthPMKSet(void)
{
	...
	if (pIoTApCfg->MBSSID.AuthMode >= Ndis802_11AuthModeWPA)
	{
		...
		RtmpPasswordHash((PSTRING)pIoTApCfg->MBSSID.Passphase, pIoTApCfg->MBSSID.Ssid, 
						 (INT)pIoTApCfg->MBSSID.SsidLen, keyMaterial);
...
VOID APCfgUpdate(UINT8 *pSSID, UINT8 AuthMode, UINT8 *pPassword, UINT8 Channel)
{
