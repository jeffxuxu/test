...
#define DEFAULT_VENDOR_ID			"0001"
#define DEFAULT_VENDOR_NEME			"socland"
#define DEFAULT_PRODUCT_TYPE    	"1002"
#define DEFAULT_PRODUCT_NAME        "XLITE_IOT"
#define DEFAULT_DEVICEID			"XLITE_IOT_0001"
#define DEFAULT_SSID				"XLITE_LINK"
...
#define DEFAULT_UART_BAUDRATE       UART_BAUD_9600
...
#define DEFAULT_IOT_SERVER_INTERVAL	100
#define DEFAULT_SERVER_DOMAIN		"mt.xlite.net"
#define DEV_SERVER_DOMAIN		"mttest.wulianda.net"


#define DEFAULT_IOT_CMD_PWD         {0xDE,0xFA,0xC4,0x44,0x47,0xB5,0x7F,0x15,0x2D,0x14,0xF3,0x0C,0xEA,0x7A,0x73,0xCB}




/*this is the Common CFG region default table */
const IOT_COM_CFG Com_Cfg = {
						DEFAULT_BOOT_FW_IDX,
...
{
	/*customer can move PMK calcuate process to here*/
}
...
VOID IoT_Cust_Sub_Task(VOID)
{
...
#if (IOT_SMP_RSSI_COLLECT == 1)
    {
		...
		if (((CurTime - PreTime) >= IOT_SMP_RSSI_COLLECT_PERIOD ) ||
			 (CurTime < PreTime))
		{
			PreTime = CurTime;
...
VOID IoT_Cust_Ops(VOID)
{
	...
	IoTCustOp.IoTCustScanDone = IoT_Cust_Scan_Done;
#endif
...
BOOLEAN load_usr_cfg(VOID)
{
	...
	if (IoTpAd.flash_rw_buf[FLASH_USR_CFG_PRODUCT_INFO_STORED] == PRODUCT_INFO_STORED)
	{
		...
		memcpy( IoTpAd.UsrCfg.VendorID,	
				&IoTpAd.flash_rw_buf[FLASH_USR_CFG_VENDOR_ID],	
			    FLASH_USR_CFG_RESERVE_1 - FLASH_USR_CFG_VENDOR_ID);
#if __PID_GATEWAY__
		memcpy(&gWifiOutletCfg.OutletCoding, &IoTpAd.flash_rw_buf[FLASH_USR_CFG_OUTLETCODING], FLASH_USR_CFG_OUTLETCODING_LEN);
		memcpy(&gWifiOutletCfg.Group, &IoTpAd.flash_rw_buf[FLASH_USR_CFG_GROUP], FLASH_USR_CFG_GROUP_LEN);
		memcpy(gWifiOutletCfg.Member, &IoTpAd.flash_rw_buf[FLASH_USR_CFG_MEMBER], FLASH_USR_CFG_MEMBER_LEN);
		memcpy(gWifiOutletCfg.LightType, &IoTpAd.flash_rw_buf[FLASH_USR_CFG_LIGHTTYPE], FLASH_USR_CFG_LIGHTTYPE_LEN);

		spi_flash_read(FLASH_USR_CFG_BASE+FLASH_USR_CFG_RULEID, IoTpAd.flash_rw_buf, sizeof(IoTpAd.flash_rw_buf));
		memcpy(&gIoTRuleCfg.gRuleID, &IoTpAd.flash_rw_buf[FLASH_USR_CFG_RULEID-FLASH_USR_CFG_RULEID], FLASH_USR_CFG_RULEID_LEN);
		memcpy(gIoTRuleCfg.RuleValue, &IoTpAd.flash_rw_buf[FLASH_USR_CFG_RULEVALUE-FLASH_USR_CFG_RULEID], FLASH_USR_CFG_RULEVALUE_LEN);
		memcpy(gIoTRuleCfg.RuleTODO, &IoTpAd.flash_rw_buf[FLASH_USR_CFG_RULETODO-FLASH_USR_CFG_RULEID], FLASH_USR_CFG_RULETODO_LEN);
		memcpy(gIoTRuleCfg.RulePeriod, &IoTpAd.flash_rw_buf[FLASH_USR_CFG_RULEPERIOD-FLASH_USR_CFG_RULEID], FLASH_USR_CFG_RULEPERIOD_LEN);
#endif
	#else
		/*if flash stored valid vendor name , product name and product type*/
		memcpy(IoTpAd.UsrCfg.VendorID,	&IoTpAd.flash_rw_buf[FLASH_USR_CFG_VENDOR_ID],	FLASH_USR_CFG_VENDOR_ID_LEN);
		...
		memcpy(IoTpAd.UsrCfg.DeviceID, 	&IoTpAd.flash_rw_buf[FLASH_USR_CFG_DEVICE_ID],	FLASH_USR_CFG_DEVICE_ID_LEN);
	#endif
	}
#else
...
BOOLEAN reset_usr_cfg(BOOLEAN bUpFlash)
{
	...
	if(bUpFlash == TRUE)
	{
		...
		memcpy( &IoTpAd.flash_rw_buf[FLASH_USR_CFG_VENDOR_ID],	
			    IoTpAd.UsrCfg.VendorID,
			    FLASH_USR_CFG_RESERVE_1 - FLASH_USR_CFG_VENDOR_ID);
#else
		memcpy(&IoTpAd.flash_rw_buf[FLASH_USR_CFG_VENDOR_ID] ,  Usr_Cfg.VendorID, FLASH_USR_CFG_VENDOR_ID_LEN);
		...
		memcpy(&IoTpAd.flash_rw_buf[FLASH_USR_CFG_DEVICE_ID] , Usr_Cfg.DeviceID, FLASH_USR_CFG_DEVICE_ID_LEN);
#endif
...
#if __PID_GATEWAY__
	memcpy( &IoTpAd.flash_rw_buf[FLASH_USR_CFG_RESERVE_1],	
		    &gWifiOutletCfg.OutletCoding,
		    FLASH_USR_CFG_RESERVE_2 - FLASH_USR_CFG_RESERVE_1);	
	spi_flash_write(FLASH_USR_CFG_BASE, IoTpAd.flash_rw_buf, sizeof(IoTpAd.flash_rw_buf));	

	
	memset(IoTpAd.flash_rw_buf ,0xFF, sizeof(IoTpAd.flash_rw_buf));
	memcpy( &IoTpAd.flash_rw_buf[0],	
		    &gIoTRuleCfg.gRuleID,
		    FLASH_USR_CFG_RESERVE_3 - FLASH_USR_CFG_RULEID);
	spi_flash_write(FLASH_USR_CFG_BASE+FLASH_USR_CFG_RULEID, IoTpAd.flash_rw_buf, sizeof(IoTpAd.flash_rw_buf));	
#endif
}
...
BOOLEAN default_ip_cfg(VOID)
{
	...
	memcpy( &IoTpAd.ComCfg.TCPUDP_CS,		
		    &Com_Cfg.TCPUDP_CS, 
		    (IoTpAd.ComCfg.IoT_ServeDomain+ FLASH_COM_CFG_IOT_SERVER_DOMAIN_LEN - &IoTpAd.ComCfg.TCPUDP_CS));
	...
	memcpy(IoTpAd.ComCfg.IoT_ServeIP,	Com_Cfg.IoT_UDP_Srv_Interval,	FLASH_COM_CFG_IOT_SERVER_INTERVAL_LEN);	
	memcpy(IoTpAd.ComCfg.IoT_ServeDomain,	Com_Cfg.IoT_ServeDomain,	FLASH_COM_CFG_IOT_SERVER_DOMAIN_LEN);
#endif
...
BOOLEAN load_com_cfg(VOID)
{
	...
	if (IoTpAd.flash_rw_buf[FLASH_COM_CFG_INFO_STORED] == COMMON_INFO_STORED)
	{
		...
		memcpy( &IoTpAd.ComCfg.TCPUDP_CS,
				...
				FLASH_COM_CFG_IOT_SERVER_DOMAIN + FLASH_COM_CFG_IOT_SERVER_DOMAIN_LEN - FLASH_COM_CFG_TCPUDP_CS_SET);
	#else
		...
		memcpy(IoTpAd.ComCfg.IoT_ServeDomain,	&IoTpAd.flash_rw_buf[FLASH_COM_CFG_IOT_SERVER_DOMAIN], 	FLASH_COM_CFG_IOT_SERVER_DOMAIN_LEN);
	#endif	
		...
		if( ((IoTpAd.ComCfg.IoT_TCP_Srv_Port == 0) || (IoTpAd.ComCfg.IoT_TCP_Srv_Port == 0xFFFF)) ||
			...
			(check_data_valid(IoTpAd.ComCfg.IoT_ServeDomain,  FLASH_COM_CFG_IOT_SERVER_DOMAIN_LEN) == FALSE))
		{
			default_ip_cfg();
		}
#ifndef RELEASE		
		memset(IoTpAd.ComCfg.IoT_ServeDomain,0,FLASH_COM_CFG_IOT_SERVER_DOMAIN_LEN);
		memcpy(IoTpAd.ComCfg.IoT_ServeDomain,DEV_SERVER_DOMAIN,strlen(DEV_SERVER_DOMAIN));
#endif
...
BOOLEAN reset_com_cfg(BOOLEAN bUpFlash)
{
	...
	if(bUpFlash == TRUE)
	{
		...
		memcpy(&IoTpAd.flash_rw_buf[FLASH_COM_CFG_IOT_SERVER_DOMAIN],	Com_Cfg.IoT_ServeDomain,	FLASH_COM_CFG_IOT_SERVER_DOMAIN_LEN);
...
void restore_com_cfg()
{
	memset(IoTpAd.flash_rw_buf ,0xff, sizeof(IoTpAd.flash_rw_buf));
	IoTpAd.flash_rw_buf[FLASH_COM_CFG_INFO_STORED]    = COMMON_INFO_STORED;
	IoTpAd.flash_rw_buf[FLASH_COM_CFG_BOOT_IDX]=IoTpAd.ComCfg.BootFWIdx;
	IoTpAd.flash_rw_buf[FLASH_COM_CFG_RECOVERY_MODE_STATUS]=IoTpAd.ComCfg.RecovModeStatus;
	IoTpAd.flash_rw_buf[FLASH_COM_CFG_IO_SET]=IoTpAd.ComCfg.IOMode;

	memcpy( &IoTpAd.flash_rw_buf[FLASH_COM_CFG_UART_BAUD],
			&IoTpAd.ComCfg.UART_Baudrate, 
			FLASH_COM_CFG_RESERVE_2 - FLASH_COM_CFG_UART_BAUD);
	
	memcpy( &IoTpAd.flash_rw_buf[FLASH_COM_CFG_TCPUDP_CS_SET],
			&IoTpAd.ComCfg.TCPUDP_CS,
			FLASH_COM_CFG_IOT_SERVER_DOMAIN - FLASH_COM_CFG_TCPUDP_CS_SET);
...
{
#ifdef  CONFIG_STATION  /*Smnt is valid in STA mode, and should not available in SoftAP mode*/
