...
#define MAC_IP_LEN				4
#define MAC_DOMAIN_NAME_LEN		(64)

#define EFUSE_SERIAL_CODE_OFFSET		0x52
#define SERAIL_CODE_LEN					12
#define DEVICE_ID_LEN			16
#define MCU_VERCODE_LEN			10
#define MSG_SIGN_LEN			16
#define IMEI_SIGN_LEN			16
#define MAGIC_LEN				4
#define FLASH_MAC_ADDR_OFFSET	0x17004
...
#define OPERATE_RESP_DATALEN	1024
...
typedef enum{
	SHCMD_lGT_BRIGHTNESS=1,
	SHCMD_LGT_COLORTEMP,
	SHCMD_LGT_RGB,
	SHCMD_LGT_POWER,
	SHCMD_LGT_STATUS_QUERY,
	SHCMD_LGT_STATUS_RESP,
	SHCMD_LGT_UNION_SET,
	SHCMD_MCU2WIFI_ACK=0x55,
	SHCMD_WIFI2MCU_ACK=0xAA,
	SHCMD_LEARNING=0xC6,
	SHCMD_OFFLINE=0xC8,
	SHCMD_DEFAULT,
	SHCMD_VERCODE=0xCC,
	SHCMD_WOL_POWER=0x11,
	SHCMD_WOL_STATUS_QUERY=0x12,
	SHCMD_WOL_STATUS_RESP=0x13,
	SHCMD_WOL_STATUS_INDICATORLIGHT=0x14,
	SHCMD_WOL_CONTROL=0x1B,
	SHCMD_WOL_MEMBERADDRCHANGE=0x1C,
	SHCMD_WOL_LEARNIG=0x1D,
	SHCMD_WOL_DLE_CODING=0x1E,
	SHCMD_WOL_CODING=0x1F,
} SHCMD_TYPE;
...
typedef struct GNU_PACKED _IOT_COMMON_CFG_{
	...
	UINT16	IoT_UDP_Srv_Interval;	
	UINT8	IoT_ServeDomain[FLASH_COM_CFG_IOT_SERVER_DOMAIN_LEN];
...
typedef struct GNU_PACKED _IOT_USER_CFG_{
	UCHAR VendorID[FLASH_USR_CFG_VENDOR_ID_LEN];
	...
	UCHAR DeviceID[FLASH_USR_CFG_DEVICE_ID_LEN];
}IOT_USR_CFG;
...
typedef struct GNU_PACKED t_RGBColor
{
	UINT_8 red;
	UINT_8 green;
	UINT_8 blue;
...
typedef struct GNU_PACKED t_IoTLightStatus
{
	INT_32	chid;
	UINT_8 power;
	UINT_8 brightness;
...
typedef struct GNU_PACKED t_IoTWifiOutletStatus
{
	UINT_8 TrueOrFlase;
	INT32 pt;
	INT_32 pID;
	...
	UINT_8 MemberAddr;
	UINT_8 MemberType;
	UINT_8 NewMemberAddr;
	UINT_8 NewGroupAddr;
}IoT_WifiOutletStaus;


typedef struct GNU_PACKED t_IoTWifiOutletCfg
{
	UINT_16 OutletCoding;
	UINT_8 Group;
	UINT_32 Member[8];
	UINT_32 LightType[8];
}IoTWifiOutletCfg;

typedef struct GNU_PACKED t_IoTRuleCfg
{
	UINT_16 gRuleID;
	UINT_32 RuleValue[16];
	UINT_32 RuleTODO[16]; 
	UINT_8 RulePeriod[16];
}IoTRuleCfg;

//规则之一，定时器
#if 0
typedef struct GNU_PACKED t_IoTRuleTimer
{
	UINT_16 gRuleTimerID;
	UINT_32 RuleTimerValue[16];
	UINT_32 RuleTimerTODO[16]; 
}IoTRuleTimer;
#endif

typedef struct GNU_PACKED t_IoTRuleStaus
{
	UINT_8 RuleType;
	CHAR RuleOperate;
	UINT_8 TempRuleID;
	UINT_32 TempRuleValue;
	UINT_32 TempRuleTODO;
	UINT_8 TempPeriod;
}IoTRuleStaus;
...
extern char gCurrentMAC[MAC_ADDR_LEN*3];
extern CHAR gSerialCode[SERAIL_CODE_LEN+1];
extern CHAR gMCUVerCode[MCU_VERCODE_LEN+1];
...
typedef struct GNU_PACKED _STA_FOTA_CONFIG {
	UINT8 step;
	UINT8 vernumber;
	UINT16 crc16value;
	UINT32 imagesize;
	UINT8 fotado;
	UCHAR vercode[FLASH_STA_CFG_FOTA_VERCODE_LEN];
	uip_ipaddr_t notifyip;
	INT8	fotaresult;
}STA_FOTA_CONFIG;
 ...
 ******************************************************************************/
VOID sys_Init() 		 XIP_ATTRIBUTE(".xipsec0");
 ...
 ******************************************************************************/
INT16 IoT_uart_output(UCHAR *msg, INT16 count);

#if (UART_INTERRUPT == 0)
INT16 IoT_uart_input(UCHAR *msg, INT16 count);
#endif
...
UINT8 spi_flash_update_fw_done(uint8 type) XIP_ATTRIBUTE(".xipsec0");
UINT8 spi_flash_update_fw(uint8 type, uint32 offset, uint8 *pdata, uint16 len);


VOID IoT_software_pwm_handler(VOID);
VOID IoT_software_pwm_init(VOID) XIP_ATTRIBUTE(".xipsec0");
...
INT32 IoT_gpio_batch_modify_output_value(UINT32 output_bitmap, UINT32 value_bitmap);
INT32 IoT_gpio_batch_modify_mode(UINT32 output_bitmap);
...
/*========================================================================
	...
	[OUT] pGPI_INT_MODE -[b1:b0] 	  GPIO1 interrupt mode
				   		 ...
				   		 [b11:b10]	  GPIO5 interrupt mode
				   		 [b13:b12]	  GPIO6 interrupt mode
...
pQU_t GetQueueInfo(VOID);
UINT32 GetFceIntStat(VOID);
VOID StorePhyMode(UCHAR PHYMode);
...
INT  rtmp_bbp_set_bw(UINT8 bw) XIP_ATTRIBUTE(".xipsec0");
VOID AsicSwitchChannel(UCHAR Channel, BOOLEAN bScan) XIP_ATTRIBUTE(".xipsec0");
//VOID _AsicSwitchChannel(UCHAR Channel, BOOLEAN bScan) OVLY_ATTRIBUTE(".ovlysec1"); //ram0
VOID dump(PUINT8 pdata, UINT16 len);
...
VOID BeaconTimeoutAction(UINT_32 param, UINT_32 param2) XIP_ATTRIBUTE(".xipsec0");
...
=======================================================================*/
BOOLEAN PeerBeaconCHInfoParse(RX_BLK* pRxBlk) XIP_ATTRIBUTE(".xipsec0");
#endif
...
INT32 IoT_send_udp_directly(
	        ...
	        UINT16 DstPort,
	        PUCHAR pPayload, 
	        UINT16 PayloadLen) XIP_ATTRIBUTE(".xipsec0");	
...
/*========================================================================
	...
	Note: 
========================================================================*/
BOOLEAN GetClearFrameFlag() XIP_ATTRIBUTE(".xipsec0");
...
INT32 IoT_uart_AT_config(PUCHAR command_buffer, INT16 at_cmd_len);
INT32 IoT_Tcpip_AT_Connect(PUCHAR command_buffer, INT16 at_cmd_len);
INT32 IoT_Tcpip_AT_Send(PUCHAR command_buffer, INT16 at_cmd_len);
INT32 IoT_Tcpip_AT_Listen(PUCHAR command_buffer, INT16 at_cmd_len);
INT32 IoT_Tcpip_AT_Disconnect(PUCHAR command_buffer, INT16 at_cmd_len);
INT32 IoT_Tcpip_AT_Udp_Create(PUCHAR command_buffer, INT16 at_cmd_len);
INT32 IoT_Tcpip_AT_Udp_Del(PUCHAR command_buffer, INT16 at_cmd_len);
INT32 IoT_Tcpip_AT_Udp_Send(PUCHAR command_buffer, INT16 at_cmd_len);
...
INT16 IoT_parse_ATcommand(PUCHAR cmd_buf, INT16 at_cmd_len) XIP_ATTRIBUTE(".xipsec0");
INT16 IoT_parse_IWCmd(PUCHAR cmd_buf, INT16 at_cmd_len) XIP_ATTRIBUTE(".xipsec0");
...
INT16 IoT_exec_AT_cmd_uart(PUCHAR command_buffer, INT16 at_cmd_len) XIP_ATTRIBUTE(".xipsec0");
#endif

#if (ATCMD_ATE_SUPPORT == 1)
INT16 IoT_exec_AT_cmd_ATE_Cal2(PUCHAR command_buffer, INT16 at_cmd_len) XIP_ATTRIBUTE(".xipsec0");
...
INT16 IoT_exec_AT_cmd_ATE_Cal(PUCHAR cmd_buf, INT16 Len) XIP_ATTRIBUTE(".xipsec0");
#endif

#if ((ATCMD_PS_SUPPORT == 1) && (MT7681_POWER_SAVING == 1))
INT16 IoT_exec_AT_cmd_PS_Set(PUCHAR command_buffer, INT16 at_cmd_len) XIP_ATTRIBUTE(".xipsec0");
#endif

#if (ATCMD_FLASH_SUPPORT == 1)
INT16 IoT_exec_AT_cmd_Flash_Set(PUCHAR command_buffer, INT16 at_cmd_len) XIP_ATTRIBUTE(".xipsec0");
#endif

#if (ATCMD_EFUSE_SUPPORT == 1)
INT16 IoT_exec_AT_cmd_Efuse_Set(PUCHAR command_buffer, INT16 at_cmd_len)  XIP_ATTRIBUTE(".xipsec0");
#endif

#if (ATCMD_CH_SWITCH_SUPPORT == 1)
INT16 IoT_exec_AT_cmd_ch_switch(PUCHAR pCmdBuf, INT16 at_cmd_len) XIP_ATTRIBUTE(".xipsec0");

#endif

#if (ATCMD_NET_MODE_SUPPORT == 1)
INT16 IoT_exec_AT_cmd_netmode(PUCHAR command_buffer, INT16 at_cmd_len)  XIP_ATTRIBUTE(".xipsec0");
#endif


#ifdef CONFIG_SOFTAP
INT16 IoT_exec_AT_cmd_Conf_SoftAP(PUCHAR pCmdBuf, INT16 at_cmd_len)  XIP_ATTRIBUTE(".xipsec0");
#endif
...
INT16 IoT_exec_AT_Set_Smnt(PUCHAR pCmdBuf)  XIP_ATTRIBUTE(".xipsec0");
#endif

#if (ATCMD_GET_VER_SUPPORT == 1)
INT16 IoT_exec_AT_cmd_ver(PUCHAR pCmdBuf) XIP_ATTRIBUTE(".xipsec0");
#endif

#if (ATCMD_JTAGMODE_SUPPORT == 1)
VOID IoT_exec_AT_cmd_jtag_mode_switch(PUCHAR pCmdBuf, INT16 at_cmd_len) XIP_ATTRIBUTE(".xipsec0");
#endif
